...
class Sale extends Base
{
    ...
    public function update(){
        ...
        $id = $form['id'] ?? null;
        if (is_null($id)) {
            return $this->SendJson($response, ['status' => false, 'msg' => 'para alterar a venda informar o codigo'], 400);
        }
        $total_venda = SelectQuery::select("sun(total_liquido) as total_liquido,sun(total_bruto) as total_bruto")
        ->from('item_sale')
        ->where('id_venda','=', $id)
        ->fecht()
        ...
        return $this->SendJson($response, $total_venda);
    }
